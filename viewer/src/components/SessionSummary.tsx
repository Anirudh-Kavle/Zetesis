...
interface Props {
  sessionId: string;
  lastEventTs?: number; // newest event in the session — for staleness
...
export function SessionSummaryPanel({ sessionId, lastEventTs, onOpenEvent }: Props) {
  ...
  return (
    <div className="border-b border-border bg-surface px-4 py-2 text-sm">
      {state.kind === "ready" ? (
          ...
          <p className="min-w-0 leading-relaxed text-ink-muted">
            ...
            {state.summary.model && (
              <span
                ...
                title="Generated by this model running on this machine — nothing was sent to any cloud service"
              >
            ...
            {state.summary.legacy && (
              <span
                className="ml-2 font-mono text-xs text-risk-write"
                title="This summary was written by an older version without citation support — regenerate (↻) to get a verifiable, cited summary"
              >
                ⚠ old format, no citations — regenerate ↻
              </span>
            )}
            {/* Summaries are point-in-time snapshots; flag when the session
                has recorded events after this one was generated. */}
            {lastEventTs != null &&
              state.summary.generated_at != null &&
              lastEventTs > state.summary.generated_at && (
                <span className="ml-2 font-mono text-xs text-risk-write">
                  ⚠ new events since this summary
                </span>
              )}
            <button
              onClick={generate}
        ...
        <button
          onClick={generate}
          title="Summarizes this session's recorded events with a small AI model running on this machine. Free, private, ~30 seconds. Every claim gets a clickable [event] citation you can verify."
