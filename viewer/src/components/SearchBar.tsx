...
const QUALIFIER_KEYS = ["tool", "risk", "file", "session"] as const;
...
// build the same string. A few qualifiers have no dedicated column (file:,
// exit:, provider:, after:, before:) and stay type-only; the placeholder
// below names them. Esc-to-clear-search is handled globally; Esc here only
// closes the panel (see the capture-phase listener below).
      ...
      <div ref={wrapperRef} className="relative flex w-full max-w-3xl items-center gap-2">
        <div className="flex min-w-0 flex-1 items-center gap-2 rounded-md border border-border bg-surface px-3 py-1.5">
          ...
          <input
            ...
            placeholder="search all sessions…   also: exit:fail provider:claude after:2026-07-01"
            title="Searches every recorded session, not just what's loaded. Use the Filters panel for risk/tool/session, or type qualifiers directly: file: exit: provider: after: before:"
        ...
        <button
          ...
          title="Filter by risk tier, tool, or session"
